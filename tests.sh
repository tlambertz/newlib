...
echo "deb [trusted=yes] https://dl.bintray.com/hermitcore/$OS_TYPE $OS_NAME main" | tee -a /etc/apt/sources.list
apt-get update
...
make -j2
checkinstall -D -y --exclude=build --pkggroup=main --maintainer=stefan@eonerc.rwth-aachen.de --pkgsource=https://hermitcore.org --pkgname=newlib-hermit --pkgversion=2.4.0 --conflicts=newlib-hermit-rs --pkglicense=BSD-3-Clause make install

cd ..
mkdir -p tmp
dpkg-deb -R build/newlib-hermit_2.4.0-1_amd64.deb tmp
rm -f build/newlib-hermit_2.4.0-1_amd64.deb
